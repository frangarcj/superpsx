 ...
 *
 * VU0 micro programs assembled from .vsm sources via dvp-as, linked
 * directly into the ELF as .vudata section (no bin2c needed).
...
/* Extern symbols from dvp-as assembled .vsm files (linked via .vudata section).
 * Each .vsm defines a _start and _end global label around the VU0 code. */
extern unsigned char vu0_mvmva_full     __attribute__((section(".vudata")));
extern unsigned char vu0_mvmva_full_end __attribute__((section(".vudata")));
extern unsigned char vu0_mvmva_core     __attribute__((section(".vudata")));
extern unsigned char vu0_mvmva_core_end __attribute__((section(".vudata")));
...
void vu0_micro_init(void)
{
    ...
    unsigned int full_size = &vu0_mvmva_full_end - &vu0_mvmva_full;
    unsigned int core_size = &vu0_mvmva_core_end - &vu0_mvmva_core;

    /* Copy mvmva_full at offset 0x000 */
    memcpy((void *)micro_mem, &vu0_mvmva_full, full_size);

    /* Copy mvmva_core at offset 0x088 */
    memcpy((void *)(micro_mem + VU0_PROG_MVMVA_CORE), &vu0_mvmva_core, core_size);
