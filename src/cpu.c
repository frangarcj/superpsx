...
/* ---- JIT-callable overflow exception helper ----
 * Called via emit_call_c trampoline on the cold (overflow) path.
 * cpu.current_pc must be set by the JIT before the call.
 * Triggers PSX exception code 0x0C (Overflow). */
