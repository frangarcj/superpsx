...
void block_scan(const uint32_t *code, int max_insns, BlockScanResult *out)
{
    ...
    memset(out->reg_first_use, 0xFF, sizeof(out->reg_first_use));
    memset(out->reg_last_use, 0, sizeof(out->reg_last_use));
    for (int i = 0; i < count; i++)
    {
         ...
         * and track first/last use index for density-based allocation */
        ...
        while (amask)
        {
            ...
            if (out->reg_first_use[r] == 0xFF)
                out->reg_first_use[r] = (uint8_t)i;
            out->reg_last_use[r] = (uint8_t)i;
        }
    }
